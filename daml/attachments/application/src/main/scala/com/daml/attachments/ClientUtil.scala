...
import com.daml.attachments.FutureUtil.toFuture
...
class ClientUtil(
  ...
  def submitCommandAndWait[T](
                        sender: P.Party,
                        workflowId: WorkflowId,
                        command: P.Update[T])(implicit ec: ExecutionContext): Future[Transaction] = {
      ...
      SubmitAndWaitRequest(
        Some(Commands(
          ...
          Some(fromInstant(now)),
           Some(fromInstant(now.plusNanos(ttl.toNanos))),
  ...
  def acsSnapshot(party: P.Party): Source[GetActiveContractsResponse, Future[String]] =
    client.activeContractSetClient.getActiveContracts(transactionFilter(party), false)

  def subscribeSource(party: P.Party, offset: LedgerOffset)(implicit mat: Materializer): Source[Transaction, NotUsed] = {
    transactionClient.getTransactions(offset, None, transactionFilter(party))
  }
