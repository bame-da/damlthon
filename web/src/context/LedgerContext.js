...
export function getClaims(state) {
  const auth = getContract(state, "Fact.Auth", "Auth");
  const claims = auth ? Object.keys(auth.argument.claims.textMap).map(k => k.split(";")[0]) : [];
  return claims;
}

export function hasClaim(state, claim) {
  const claims = getClaims(state);
  return claims.indexOf(claim) >= 0;
}
